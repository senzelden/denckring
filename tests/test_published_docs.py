...
#: Directories under `docs/` that are published on purpose. Every one of them is
#: reachable from `mkdocs.yml`'s nav — `adr/` entry by entry, the other three through
#: an index page.
#:
#: This list exists to make the *complement* assertable, which is the only form of this
#: check that catches a directory nobody has thought about yet. Naming what is published
#: and what is excluded turns "is this directory listed?" into a question with no third
#: answer.
#:
#: There is deliberately no test that these directories exist. `gallery/` is gitignored
#: and generated by `scripts/build_docs.py`, so a fresh checkout does not have it until
#: the docs job runs — an existence check here would pass on a maintainer's machine and
#: be red in CI, which is precisely the failure this file was split to end. The
#: complement check below is safe in both directions because it can only fire on a
#: directory that is actually present wherever it runs.
PUBLISHED_DIRECTORIES = ("adr/", "api/", "gallery/", "showcase/")


def test_every_directory_under_docs_is_classified() -> None:
    """Published on purpose or excluded on purpose, and nothing in between.

    The lists above only ever validated directories already named in them, so a *new*
    directory under `docs/` was invisible to every check in this file: it would be
    published by mkdocs and shipped in the sdist while being linked from nothing, which
    produces no dangling link, no nav warning and no `--strict` failure.

    That happened three times on 2026-09-07 alone — `superpowers/` red in CI for a day,
    `stage_mockups/` bound for a public URL and the sdist, `feedback/` the same — and
    each was found by hand. An unclassified directory is the one failure mode the rest
    of this file cannot see, because every other test starts from a name someone already
    wrote down.

    Adding a directory under `docs/` is therefore a decision this test forces you to
    record, in one of the two lists, rather than one you can make by accident.
    """
    on_disk = {f"{path.name}/" for path in (ROOT / "docs").iterdir() if path.is_dir()}
    classified = set(PUBLISHED_DIRECTORIES) | set(WORKING_DOCUMENTS)
    unclassified = sorted(on_disk - classified)
    assert not unclassified, (
        f"docs/{unclassified} is neither published nor excluded on purpose — add it to "
        f"PUBLISHED_DIRECTORIES if the site should carry it, or to the working documents "
        f"above (and mkdocs.yml's exclude_docs, and the sdist exclusions) if it should not"
    )


def test_no_directory_is_both_published_and_excluded() -> None:
    """The two lists are answers to the same question, so an overlap is a contradiction
    the complement check above would otherwise absorb in silence: a directory in both
    sets is `classified`, and this file would go on asserting that mkdocs excludes a
    directory the nav also publishes."""
    both = sorted(set(PUBLISHED_DIRECTORIES) & set(WORKING_DOCUMENTS))
    assert not both, f"{both} is listed as both published and excluded"
