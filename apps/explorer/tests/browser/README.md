...
a check of the poem read cell by cell out of them. Its `wave` mode measures the
drum animation specifically and should be deleted with it when the board is
rebuilt as character cells; the other five modes are about the routes, the
invariant and the liveness machinery and outlive that change.
