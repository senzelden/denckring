...
Eleven Playwright scripts. They are **not** run by `pytest`, or by anything else
...
`witz-stream.mjs` is the odd one out, and worth naming as such: it drives no
device. It drives the *wait* — the reading both scene two and the bench can
ask a model for, which takes the better part of half a minute and showed an
empty box for the whole of it until `static/witz_stream.js` existed. It is
also the only one here that spends money: there is no way to check "the
indicator is up while the model thinks" against a stub, because the thing
being checked is the thinking.
...
| `witz-stream.mjs` | The wait on both pages that can ask for a reading — `stage`, `bench`, or `both`. Holds each to four things sampled *throughout* rather than checked at the end: the indicator is up within 2s of the press, it says a number, that number climbs (so a stalled stream cannot pass as a slow one), and the box is never empty with no indicator over it. Costs one real API call per page. Verified against the defect it exists for by taking `witz_stream.js` away: three of the six checks fail and the paragraph still arrives, which is the htmx fallback proving it is still there. |
