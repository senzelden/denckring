...
class Acrostic(BaseProcedure[AcrosticParams]):
    ...
    def _check(self, text: str, pack: LanguagePack, params: AcrosticParams) -> Report:
        # Flattened, not one entry per source character — `fold_target` owns
        # the expression and says why (ADR 0035, D3).
        expected = fold_target(params.target, pack, fold=params.fold_diacritics)
