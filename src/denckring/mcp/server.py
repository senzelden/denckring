...
_MAX_TEXT_CHARS_ENV = "DENCKRING_MCP_MAX_CHARS"


def _max_text_chars_from_env(raw: str | None) -> int:
    if raw is None:
        return _DEFAULT_MAX_TEXT_CHARS
    try:
        value = int(raw)
    except ValueError:
        warnings.warn(
            f"{_MAX_TEXT_CHARS_ENV} must be an integer; using {_DEFAULT_MAX_TEXT_CHARS}.",
            RuntimeWarning,
            stacklevel=2,
        )
        return _DEFAULT_MAX_TEXT_CHARS
    if value < 1:
        warnings.warn(
            f"{_MAX_TEXT_CHARS_ENV} must be positive; using {_DEFAULT_MAX_TEXT_CHARS}.",
            RuntimeWarning,
            stacklevel=2,
        )
        return _DEFAULT_MAX_TEXT_CHARS
...
MAX_TEXT_CHARS = _max_text_chars_from_env(os.environ.get(_MAX_TEXT_CHARS_ENV))
